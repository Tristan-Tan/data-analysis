...
2. 复赛数据到手后，**第一步**先统计 `testB` 的末笔日期日级分布，与 train / testA 对齐核对（A 榜实测：label0 的末笔日期全部 ≥ 窗口起点，全量末笔日最大值为 2026-01-01）；用 `code/explore/testb_anchor_check.py` 做这个核对；
3. **B 榜实测结果**：testB 末笔日期整体比 train/testA 晚约一个月（testB min=2025-12-20、p50=2026-01-27，train/testA 集中在 2025-12~2026-01-01），是两个独立快照批次的固有差异，不是几天量级的漂移。**若按原计划"统一锚点、同步重算训练侧"，实测会把 train 侧 sil_days 整体拉高约 30 天（label0 均值 7.92→37.92，sil≥30 占比 1.78%→100%），特征完全失去区分度**——原计划里"绝不可让两侧各自取自己的最大值"这条判断，在批次间隔达到月级量级时是不成立的，已被本次实测推翻。
4. **修正后的正确方案**：train+testA 与 testB **分别使用各自批次内部**的末笔日期最大值定锚（`09_silence_feat.py` 已按此重写，通过 `testb.parquet` 判断卡号归属哪个批次，train 侧锚点与数值保持 A 榜原值不变，testB 侧单独定锚），语义上仍是"该卡距其所属批次快照时间点的沉默天数"，两批次可比、且不破坏 train 侧已验证过的分布。
