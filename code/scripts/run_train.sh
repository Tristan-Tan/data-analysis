...
# 用法: bash code/scripts/run_train.sh [testa|testb]  （默认 testa）
set -e
cd "$(dirname "$0")/../.."
export PYTHONPATH="$PWD/code:$PYTHONPATH"
TARGET="${1:-testa}"
echo "本次训练目标测试集: $TARGET"
...
echo "===== 07 exp5 暴露编码 (fold-matched) ====="   ; python code/train/07_exp5_encoding.py "$TARGET"
echo "===== 07b smy_cd 目标编码 (fold-matched) ====" ; python code/train/07b_smy_te_encoding.py "$TARGET"
echo "===== 08 入金侧特征 ====="                     ; python code/train/08_inflow_feat.py
echo "===== 09 silence 特征族 ====="                 ; python code/train/09_silence_feat.py
echo "===== 10 构造 AB-thin + 训练三族模型 ====="      ; python code/train/10_train_models.py "$TARGET"
